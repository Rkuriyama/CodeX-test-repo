(() => {
  ...
  function extractLatexExpressionsFromSelection() {
    ...
    state.selectionLength = text.length;
    ...
    collectFromText(text, inlineRegex, expressions);
    collectFromText(text, displayRegex, expressions);
    collectFromText(text, bracketRegex, expressions);
    collectFromText(text, parenRegex, expressions);
    collectFromEnvironmentText(text, environmentRegex, expressions);

    return expressions;
  }

  function collectFromText(text, regex, target) {
    regex.lastIndex = 0;
    let match;
  ...
  function collectFromEnvironmentText(text, regex, target) {
    regex.lastIndex = 0;
    let match;
    while ((match = regex.exec(text))) {
  ...
  function updateAnalysis() {
    const expressions = extractLatexExpressionsFromSelection();
  ...
  function renderTable(analysis) {
    ...
    if (analysis.rows.length === 0) {
      ...
      if (!state.selectionAvailable) {
        emptyCell.textContent = '解析するテキストを選択し、「更新」ボタンを押してください。';
      } else {
        emptyCell.textContent = '選択範囲からLaTeXの変数が検出されませんでした。';
      }
  ...
  function renderStatus(expressionCount) {
    ...
    if (!state.selectionAvailable) {
      state.statusArea.textContent = 'テキストが選択されていません。解析したい範囲を選択してから更新してください。';
      return;
    }

    state.statusArea.textContent = `選択文字数: ${state.selectionLength} / 検出されたLaTeX表現: ${expressionCount}`;
  }
